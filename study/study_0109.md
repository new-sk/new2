...
- 예외3. 배타적관계 : 배송처(종류 다양 : 물류센터, 부서, 공사현장 등등), 다르지만 계속 종류가 늘어난다면 하나의 엔터티로 만들는 것이 나을 수도 있음
...
관계 정의 방법
- 관계 구문 이해
  - "엔터티1"의 "임의"의 "하나"는 "관계형태" "엔터티2" 관계명이 "관계사양"한다.
  - 관계형태(Cardinality) : 하나가 몇 개의 상대방 가질 수 있는지
  - 관계사양(Optionality) : 필수(실선) vs 선택(점선)  /  반드시 vs 일 수도 있다.
- 관계 정의 절차
  - 상세하게 다시 고민 필요 : 사원 - 부서 : 겸임 가능한지, 조직원 없는 부서 가능한지, 이전 소속도 알아야 하는지,,,

관계 형태
- 1:1 : 
  - 필수:선택 : 
  - 필수:필수 : 엔터티 수직 분할 시 많이 나타남
  - 선택:선택 : 
- M:1
  - 가장 흔한 형태로 One Optional이거나 Both Side Optional이 대부분
- M:M
  - 양쪽 모두 Optional, 빈번하게 나타나지만 물리 모델로 바뀌면 사라짐
- 다중 관계 처리 (보험계약, 고객, (보험계약관련자:계약자,피보험자,수익자))
  - 병렬식 : 테이블이 될 때마다 컬럼 추가 / 인덱스 증가, SQL 복잡 / 변경에 취약한 구조
  - 직렬식 : 새로운 엔터티 필요, 관계들이 Row 형태로 나타남
- 특수한 관계 형태 (순환 vs BOM : 부모가 1개 vs 여러개)
  - 순환관계
  - BOM : 부품과 조립규칙, 2개의 1:M 관계임
  - Arc(mutually Exclusive) : 관계가 보통 동일하다, 하나의 엔터티에만 속해야 한다 
