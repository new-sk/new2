...
# 파일읽기
df = pd.read_csv(my_dir + '/my1_input_group.txt', dtype={0: str})

# 첫 번째 컬럼 값이 0이 아닌 행만 필터링
df_filtered = df[df.iloc[:, 0] == "0"]
# 첫 번째 컬럼에 "G"를 추가
df_filtered.iloc[:, 0] = "GG1" + df_filtered.iloc[:, 0].astype(str)
# 첫 번째 컬럼으로 오름차순 정렬
...
df_subset.to_csv(my_dir + '/my10_input_group_ggcd.txt', index=False, header=True, quoting=1)  # quoting=1은 각 값을 큰따옴표로 묶는 옵션
